...
func shouldSkipSlaveTest(t *testing.T) {
	...
	if !strings.HasPrefix(string(outtrim), "shared") {
		t.Skipf("root propagation mode needs to be shared or shared,slave. But currently it is '%s'", outtrim)
	}
}

func TestRootfsPropagationSlave(t *testing.T) {
	shouldSkipSlaveTest(t)
	testRootfsPropagationSlave(t)
}
...
// Launch container with rootfsPropagation in container-slave mode. Also
...
// Launch container with rootfsPropagation in container_shared mode. Also
// bind mount a volume /mnt1host at /mnt1cont at the time of launch. Now do
// a mount in container (/mnt1cont/mnt2cont) and this new mount should
// propagate to host (/mnt1host/mnt2cont)
...
func TestRootfsPropagationSharedMount(t *testing.T) {
	...
	config.RootfsMountPropagation = configs.MNT_RSHARED
	...
	// Provide SYS_ADMIN
	processCaps := append(config.Capabilities, "SYS_ADMIN")
